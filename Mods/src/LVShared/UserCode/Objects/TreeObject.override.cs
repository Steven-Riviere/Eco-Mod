...
// Le Village - ajout talent niveau 6 de l'abatteur
// Le Village - gestion du stack pour le talent de diététique qui ajoute x% d'objets - inspiration du mod XP Benefit
...
namespace Eco.Mods.Organisms
{
    ...
    [Serialized]
    public partial class TreeEntity : Tree, IDamageable, IHasInteractions
    {
                ...
                for (var z = -radius; z <= radius; z++)
                {
                    ...
                    if (!aboveblock.Is<Solid>())
                    {
                        ...
                        if (hitblock.Is<Solid>() && World.GetBlock(abovepos).Is<Empty>() && RandomUtil.Value < this.Species.ChanceToSpawnDebris)
                        {
                            ...
                            //Le Village - Chance de récupérer les copeaux
                            if (player.User != null)
                            {
                                ...
                                {
                                    //Le village - Code d'origine donc quand le talent ne s'applique pas
        ...
        private GameActionPack TryDamageTrunk(GameActionPack pack, INetObject damager, float amount, Item tool)
        {
            ...
            pack.AddPostEffect(() =>
            {
                ...
                if (this.health <= 0)
                {
                    ...
                    //Le village - Chance de supprimer les racines une fois l'arbre tombé
                    if (user != null)
                    {
                        if (user.Talentset.HasTalent(typeof(LoggingLoggersLuckTalent)))
                        {
                            //Reprise en partie du code existant pour le Stump (voir plus bas dans le code d'origine)
